...
public class SvmpI {
    ...
    private SvmpI(Map<List<AminoAcid>, Double> peptides, boolean defaultSvm) throws Exception {
        ...
        if(!defaultSvm && peptides != null){
            ...
            currentClassifier = getBestClassifier(pIDataSet);
        }else{
   ...
   // @Override
    ...
    public SvmClassifier getBestClassifier (BuildPIDataSet PIData) throws Exception {

        SvmClassifier svm;

        for(int i = 0; i < countModels; i++){

            PIData.randomizeDataSet();
            /*Randomize the dataset each time that a new Classifier is created and Study*/

            Instances train = PIData.getTrainDataset(5,0);
            Instances test = PIData.getTestDataset(5,0);
            ...
            RBFKernel rbf = SvmKernel.getInstanceRBFKernel(train, cache, gamma);
     ...
     *
     * @param sequence Peptide or PIProteinDB. The list of AA in the sequence.
     */
